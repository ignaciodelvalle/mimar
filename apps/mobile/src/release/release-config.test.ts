...
describe("Android hardening (N1)", () => {
  // Decisions 4A/5: no AsyncStorage content (drafts, session caches) may leave
  // the device in a Google Drive Auto Backup. `allowBackup: false` is the
  // whole mechanism — see app.config.ts's comment on the android block for
  // why the leftover expo-secure-store backup-rules XML is harmless once this
  // is off.
  it("turns off Android auto-backup", () => {
    expect(resolved.android?.allowBackup).toBe(false);
  });

  it("blocks the two unused permissions with no cost to block", () => {
    // Confirmed against every dependency's own AndroidManifest.xml on
    // 2026-09-24: nothing in this app's dependency graph requests either.
  ...
  it("never blocks the storage permissions the legacy photo picker needs below API 33", () => {
    // expo-image-picker's own manifest already scopes READ/WRITE_EXTERNAL_STORAGE
    // to `maxSdkVersion="32"`. Adding either to `blockedPermissions` would strip
    // it unconditionally (`tools:node="remove"` ignores maxSdkVersion) and break
    // photo selection on the J7 field devices, which run Android 8/9 (API
    // 26/28) — well under 33, where the permission-free system Photo Picker
    // does not exist yet.
