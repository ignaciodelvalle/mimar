...
function harness(dirty: boolean, onDiscard?: () => void) {
  let handler: Handler = () => {};
  const navigation = {
  ...
  let allowLeave: () => void = () => {};
  function Probe() {
    allowLeave = useDiscardGuard(navigation, dirty, undefined, onDiscard).allowLeave;
    return null;
  }
  render(<Probe />);
  return {
...
describe("useDiscardGuard", () => {
  ...
  it("fires the 4th arg's onDiscard the instant Salir is confirmed, BEFORE the dispatch (Re-2, decision 16A)", () => {
    const alert = jest.spyOn(Alert, "alert").mockImplementation((_t, _m, buttons) => {
      const salir = buttons?.find((b) => b.text === "Salir");
      salir?.onPress?.();
    });
    const order: string[] = [];
    const onDiscard = jest.fn(() => order.push("discard"));
    const h = harness(true, onDiscard);
    const originalDispatch = h.navigation.dispatch;
    h.navigation.dispatch = jest.fn((action) => {
      order.push("dispatch");
    ...
    // BEFORE, not merely called — `alta.tsx` uses this ordering to wipe the
    // draft ahead of the navigation, and a swap here would let a write queued
    // by the departing screen race the delete.
    expect(order).toEqual(["discard", "dispatch"]);
    alert.mockRestore();
  });

  it("does not call onDiscard when the person stays", () => {
    const alert = jest.spyOn(Alert, "alert").mockImplementation((_t, _m, buttons) => {
