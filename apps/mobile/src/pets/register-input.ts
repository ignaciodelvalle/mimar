...
/**
 * `AR-S` → `"Santa Fe"`. ONE COPY (the header's own rule): `PROVINCES` is
 * `@dim/contract/reference`'s catalog, the same 24 rows `LocalityPicker.tsx`
 * builds its own `PROVINCE_NAME` map from — a second hand-written table here
 * would be the exact drift this package exists to prevent.
 *
 * F-4 (2026-09-24 review): the confirm step showed the raw ISO code
 * (`"AR-S"`), which is what the wire carries and not what a person reads.
 * Falls back to the code itself for anything outside the 24 rows, which
 * cannot happen from this wizard's own picker but must not throw if it ever
 * does — a confirm screen is the wrong place to surface a contract mismatch.
 */
