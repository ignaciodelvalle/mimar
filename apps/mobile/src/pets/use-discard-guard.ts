...
export function useDiscardGuard<A>(
  ...
  /**
   * Fired the instant "Salir" is confirmed, before the navigation dispatches
   * (Re-2, decision 16A). `alta.tsx` uses it to wipe the wizard's persisted
   * draft: this dialog already tells the person "Lo que cargaste hasta acá se
   * pierde", so confirming it IS the explicit discard — a draft that survived
   * past this point would make that sentence false. Read through a ref rather
   * than added to the effect's deps, the same reason `onRestore` is a ref in
   * `use-event-draft.ts`: a new function identity every render must not tear
