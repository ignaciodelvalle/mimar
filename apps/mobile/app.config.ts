...
export default ({ config }: ConfigContext): ExpoConfig => ({
  ...
  android: {
    // `allowBackup: false` and `blockedPermissions` are declared in app.json,
    // not here — this comment documents them here because the spread hides
    // them from a reader who only opens this file.
    //
    // WHY `allowBackup: false` (N1, decisions 4A/5). Auto Backup for Apps is
    // Android's default: with no opinion at all, the OS copies this app's
    // private storage — including AsyncStorage, where drafts and session
    // caches live — to the user's Google Drive. That is a copy of app data
    // leaving the device into a cloud store this app does not control, for no
    // benefit this offline-tolerant, re-login-cheap app needs. Setting it
    // false turns the attribute off outright, which is why the
    // `fullBackupContent`/`dataExtractionRules` XML that expo-secure-store's
    // own plugin still writes (for Android 12+ backup) is harmless: those
    // files only matter when backup is allowed in the first place.
    //
    // WHY `blockedPermissions` NAMES ONLY TWO OF THE FOUR THE TASK NAMED.
    // WRITE_SETTINGS and SYSTEM_ALERT_WINDOW are blocked because nothing in
    // this app or its dependencies requests them (checked against every
    // dependency's AndroidManifest.xml, 2026-09-24) — blocking an unused
    // permission has no cost.
    //
    // READ_EXTERNAL_STORAGE and WRITE_EXTERNAL_STORAGE are DELIBERATELY NOT
    // blocked. expo-image-picker's own manifest already declares both with
    // `android:maxSdkVersion="32"` — they apply only below Android 13, where
    // the system Photo Picker (permission-free) does not exist yet and the
    // library falls back to the legacy storage permission to read the
    // gallery. The J7 field devices run Android 8/9 (API 26/28), squarely in
    // that range. `blockedPermissions` does not respect `maxSdkVersion`: it
    // writes `tools:node="remove"`, which strips the permission unconditionally
    // and would silently break photo selection on exactly those phones.
    // Verified by running `expo prebuild --platform android` into a scratch
    // copy: the generated manifest keeps both permissions with
    // `maxSdkVersion="32"` intact and blocks only WRITE_SETTINGS and
    // SYSTEM_ALERT_WINDOW.
    ...config.android,
