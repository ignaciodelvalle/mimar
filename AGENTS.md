...
| **Res. SENASA 153/2021**         | Enfermedades de denuncia obligatoria del lado animal (nación). Grupo I: notificación inmediata, dentro de las 24 h de la sospecha, a SENASA; Grupo II: según la norma específica. Su art. 22 **deroga la Res. SENASA 422/2003**. | Ancla de carbunco (Grupo I, 24 h, destino SENASA, sólo veterinario o laboratorio) y del lado nacional de tuberculosis (Grupo II, sin horas). Ningún código cita ya la 422/2003 (`eno-catalog.ts`, `disease-legal-anchors.ts`). |
| **Res. CVPBA 05/2020**           | ENO en pequeños animales, PBA: brucelosis canina, clamidiosis aviar, dirofilariosis, esporotricosis, leishmaniasis visceral, leptospirosis, micobacteriosis, rabia, SARS-CoV-2 (sospecha). Plazo único: "inmediata". Canal: Centro de Zoonosis municipal → Zoonosis Urbanas PBA → SNVS 2.0. Obliga sólo a matriculados de PBA. | "Inmediata" = **24 h** en el catálogo ENO; fuera de PBA el plazo queda "a confirmar". Clamidiosis aviar fuera de alcance (el registro no toma aves); SARS-CoV-2 no incorporado (a confirmar vigencia con el CVPBA). |
...
**The rule**: when you delete or reroute a flow, `rg` its name (route, redirect, entry point) across **comments**, not just code. The justification for someone else's `if` is written in prose, so it will not show up in a type error, a test, or any of the <!-- fact:verify_fences -->83<!-- /fact --> fences.
...
| `outbreak_signal`             | system | `{ source_symptom_event_id, disease_code, disease_label, match_strength: {high_count, medium_count, low_count, matched_symptom_codes}, pet_jurisdiction_country, pet_jurisdiction_province?, pet_jurisdiction_locality?, pet_species, triggered_by?: matcher\|direct_diagnosis, place? }` — emitted when `symptom_observed` (libreta or welfare report) triggers a reportable-disease match (`matcher`), or when a vet records a notifiable `disease_diagnosis` (`direct_diagnosis`). A `matcher` signal pages the authority in-app but is **never a legal ENO row** (PO S1, 2026-09-26); the authority notice routes to the signal's `place`, else the pet's home (PO S10). Owner does not see this in the libreta |
...
- ENO-notification SLA — **A7**, `fetchEnoSla` over `event_notification_outbox` (`target_kind='eno_authority'`): on-time %, breached-row count, median latency. Measures OUR outbox pipeline, not external delivery. "Done" = `delivered` (a real receiver, none exists yet) or `received` (the authority marked it, PO S3). Note: the outbox rules write `govt_webhook` rows, so A7 reads no rule-written row today — an open gap, not a zero
- **The ENO legal queue (health chain, PO S1–S10, 2026-09-26).** A legal row (`event_notification_outbox`, `lib/events/event-outbox-rules.ts`) comes only from a vet/lab `disease_diagnosis`, a positive rabies close, or a `death_recorded` with a notifiable disease recorded by a matriculated vet or lab-confirmed — never from owner or witness text (S1, S4, S7). The deadline runs from the diagnosis/occurrence date, capped at entry time, so a late entry lands overdue (S5). Non-rabies rows route to the event's `place` (unresolved → its province), never the pet's home when a place exists (S10; `lib/events/event-place-target.ts`); rabies stays one record per animal and bite case. A corrected diagnosis re-evaluates the queue: create or tighten, never lengthen or delete (S9; `lib/events/event-outbox-reevaluate.ts`). The daily drainer never marks a row delivered: with no receiver it records its pass and the row stays `pending` until the authority clicks "Marcar recibido" on `/gob/outbox` (`received`, who/when + `eno_notification_received` audit; migration 0264, S3). Stigma-sensitive diseases never trigger an automatic owner alert (S8). The vet files the diagnosis from the clinical record (`/mis-mascotas/…/clinico/diagnostico`) or the clinic panel's Atender (S2) — on ANY animal, their own included (PO decision 2026-09-26, "sin distinción": the matrícula is the gate, not the relation to the animal). A more urgent correction re-opens a notice already received — pending, receipt cleared, `eno_notification_reopened` in the audit log (migration 0265); receipts render only on `received` rows. The catalogue follows the norms in force: Res. SENASA 422/2003 is derogated; CVPBA "inmediata" = 24 h; hidatidosis 48 h flagged "a confirmar"; carbunco goes to SENASA and is vet/lab only (`src/modules/surveillance/domain/eno-catalog.ts`)
...
| ✅ | Bridge a pet_events (`maltreatment_reported`, `abandonment_reported`, `symptom_observed`) cuando subject es registered pet; los síntomas pasan por el detector y levantan `outbreak_signal` (señal para la autoridad, sin aviso ENO ni alerta al tutor — PO S7) | server-side en `src/modules/welfare/actions.ts` + `src/modules/events/application/surveillance/welfare-symptom-signals.ts` |
...
`pnpm verify` is `typecheck && lint` + **every `lint:*` fence in package.json** (<!-- fact:verify_fences -->83<!-- /fact --> — counted out of the `verify` script itself by `pnpm facts:write`; package.json remains the source of truth and this marker is regenerated from it) + `build`, in
...
| **Cada tabla nueva de `public` se declara contra los dos RPC** | `pnpm lint:subject-rights` → `scripts/check-subject-rights-coverage.ts`. Cuatro listas, verificación en las dos direcciones contra `pg_get_functiondef`. **Prueba mención, no corrección del predicado.** 17 tablas siguen en `KNOWN_GAP` — la 0207 cerró `libreta_share_tokens`, la 0208 cerró las tres suyas, la 0226 cerró `notification_dead_letter` y la 0250 sumó `place_resolutions` (solo agregado: la baja no la alcanza). **No transcribas este número a ningún otro lado: sale de `pnpm lint:subject-rights`, que lo computa desde la lista viva, y esta celda es el único lugar del documento que lo escribe.** Que siga diciendo la verdad lo cerca `__tests__/documented-subject-rights-counts.test.ts`, que lee esta línea y la compara contra `Object.keys(KNOWN_GAP).length` |
