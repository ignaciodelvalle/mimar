-- ────────────────────────────────────────────────────────────────────────────
-- 0258_grant_unit_audit_action.sql
-- Confirming a govt grant onto an authority unit enters the audit register.
--
-- WHAT IT ADDS (localidades-por-id D2, the partial-grant confirm flow)
-- ---------------------------------------------------------------------------
--   govt_assignment_unit_confirmed   a platform admin moved a govt user's
--                                    grants from their (province, locality)
--                                    name pair onto an authority unit. The
--                                    payload names the unit, the grants, every
--                                    locality the unit ADDS to them (a partial
--                                    holder is widened only by this explicit
--                                    act) and the reason; before/after carry
--                                    authority_unit_id.
--
-- WHY. Setting govt_assignments.authority_unit_id changes what an operator
-- sees and is paged for (govt_scope, 0257). The seed only LISTS partial
-- grants; a person decides, and this row says who, when and why.
--
-- Postgres cannot add a value to a CHECK: it is dropped and rewritten whole,
-- from AUDIT_LOG_ACTIONS (db/schema.ts), alphabetically, the way 0256 did.
