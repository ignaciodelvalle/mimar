-- Migration 0226 — the erasure reaches the notification dead letter, and a
-- resolved dead letter stops keeping its payload.
--
-- WHAT WAS WRONG (fresh review 2026-09, A06-G2 + A06-G1)
-- ---------------------------------------------------------------------------
-- `createNotification` writes the whole insert `values` object to
-- `notification_dead_letter.payload` when the insert throws, and the drain cron
-- (app/api/cron/drain-notification-dead-letter) replays it verbatim. Two
-- consequences, both about personal data outliving its purpose:
--
--   · A06-G2 — `erase_subject_data` redacts `notifications` for the subject and
--     never touched this table. The profile is soft-deleted, so the FK still
--     resolves, and the next drain re-created the notification the erasure had
--     just scrubbed — title, body and a finder's phone number included.
--   · A06-G1 — the drain stamps `resolved_at` and keeps `payload` forever. A
--     delivered notification's copy has no reason to exist twice, and nothing
--     in `data-lifecycle` ever deletes a dead-letter row.
--
-- WHAT THIS DOES
-- ---------------------------------------------------------------------------
-- PART A — erase_subject_data: 0222's definition verbatim plus
--   · DECLARE `dead_letters_redacted`;
--   · the redaction, immediately after the notifications one it completes;
--   · one new key in the subject_erasure audit payload.
--   Inside the RPC, so it is the SAME transaction as the rest of the erasure.
--
-- PART B — every dead letter ALREADY resolved loses its payload, the same
--   redaction the drain now applies at the moment it resolves one
--   (route.ts). dedupe_key, error_message and the timestamps stay: they record
--   that a delivery failed and when. Unresolved rows keep their payload — it is
--   still the only copy of a notification somebody has not received yet.
--
-- The drain also skips (and redacts) a row whose recipient's profile carries
-- `deleted_at`, belt-and-braces for a row written after an erasure ran.

-- ============================================================================
-- PART A — erase_subject_data: 0222's definition plus notification_dead_letter
-- ============================================================================
...
BEGIN
  ...
  -- resolved or not, loses its payload and is marked resolved: dedupe_key,
  -- error_message and the timestamps stay, because they say that a delivery
  -- failed and when, not to whom or what. payload is NOT NULL, so the redacted
  -- value is the empty object, which the drain's validator refuses to replay.
  WITH redacted AS (
...
-- ============================================================================
-- PART B — resolved dead letters keep no payload
-- ============================================================================
