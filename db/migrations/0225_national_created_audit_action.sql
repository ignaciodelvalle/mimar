-- Migration 0225 — crear un observador nacional entra al registro.
--
-- QUÉ AGREGA
-- ---------------------------------------------------------------------------
--   institutional_national_created   un admin creó una cuenta institucional
--                                    con el rol `national` (lectura en todo
--                                    el país, ninguna escritura)
--
-- POR QUÉ
-- ---------------------------------------------------------------------------
-- El rol `national` existe desde la 0214, pero no tenía quien lo escribiera:
-- `createInstitutionalAccountForAuthority` aceptaba sólo govt y admin, así que
-- la única forma de tener un observador nacional era un UPDATE a mano, sin
-- rastro de quién lo decidió (piloto T1-P9). Ahora lo crea un admin desde
-- /admin/govts/new, y crear una cuenta que ve datos de todo el país es
-- exactamente el acto que la consulta de rendición de cuentas tiene que
-- encontrar. Una acción propia y no `institutional_govt_created` con el rol en
-- el payload: quien filtre por acción no debe confundir un observador de todo
-- el país con un funcionario municipal.
--
...
-- alfabéticamente para que el diff contra la 0224 sea una sola línea.
