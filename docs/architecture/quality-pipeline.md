...
(Biome, over `.` and `app/.well-known`) → <!-- fact:verify_fences -->78<!-- /fact -->
...
<!-- fact:lint_scripts -->78<!-- /fact --> keys in `package.json` start with
`lint:`, and <!-- fact:verify_fences -->78<!-- /fact --> of them are actually
...
`lint:*` script: <!-- fact:check_scripts -->85<!-- /fact --> files under
...
(<!-- fact:mobile_jest_files -->140<!-- /fact --> files, §4), and a config
resolution smoke check.

### Fence categories (representative, not exhaustive)

`pnpm verify`'s <!-- fact:verify_fences -->78<!-- /fact --> fences group into
...
and are the reason CI splits into a `check` job (no DB, most of the <!-- fact:verify_fences -->78<!-- /fact --> fences)
...
<!-- fact:canon_rows -->526<!-- /fact --> rows, harvested from the project's
...
- <!-- fact:canon_enforced -->184<!-- /fact --> **ENFORCED** — a fence or test
