...
Across the whole tree: <!-- fact:pages -->272<!-- /fact --> `page.tsx` files,
...
|---|---|---|
| `pnpm verify` | <!-- fact:verify_fences -->83<!-- /fact --> `lint:*` fences plus `verify:mobile` and `build` | Structure, conventions, authorization scoping, brand casing |
| `pnpm test:verified` | `scripts/run-verified-suite.ts` over <!-- fact:vitest_files -->1779<!-- /fact --> vitest files | Behaviour, with a filesystem census that distrusts vitest's exit code in both directions |
