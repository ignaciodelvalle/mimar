...
<!-- fact:route_handlers -->98<!-- /fact --> `route.ts` handlers, and only
...
|---|---|---|
| `pnpm verify` | <!-- fact:verify_fences -->79<!-- /fact --> `lint:*` fences plus `verify:mobile` and `build` | Structure, conventions, authorization scoping, brand casing |
| `pnpm test:verified` | `scripts/run-verified-suite.ts` over <!-- fact:vitest_files -->1686<!-- /fact --> vitest files | Behaviour, with a filesystem census that distrusts vitest's exit code in both directions |
...
| Mobile Jest | `apps/mobile/jest.config.js`, <!-- fact:mobile_jest_files -->140<!-- /fact --> files | The phone's own logic, no native modules |
