...
5. **`eas update --channel preview`**, with a message carrying the commit sha
   and one line of what it fixes. `eas update` exports the bundle into `dist`
   (its `--input-dir` default) and publishes exactly that directory, so step 5a
   uploads maps for the bundle that was published. There is no `--export-dir`
   flag on `eas update`; do not add one. Install on a
   device, open it twice — see `fallbackToCacheTimeout` below for why twice —
   and confirm.

   5a. **Upload the sourcemaps.** `npx sentry-expo-upload-sourcemaps dist`
       (see "Sourcemap upload" below). Skip only when `SENTRY_AUTH_TOKEN` is
       genuinely unavailable in this environment — never skip because it is
       inconvenient, since the alternative is a hotfix nobody can debug from
       its own crash reports.
6. **`eas update --channel production --environment production`**,
   same message. Ignacio-gated. The `--environment` flag is not optional: it is
   what decides which `EXPO_PUBLIC_*` values get inlined, and step 3 is what
   proves it worked.

   6a. **Upload the sourcemaps for this publish too** —
       `npx sentry-expo-upload-sourcemaps dist`. A separate run: `dist` from
       step 5 was preview's bundle, production's is a distinct build with its
       own debug IDs, and step 6 overwrote `dist` with it, so re-running step
       5a's command against the fresh `dist` is not optional duplication.
...
### Sourcemap upload (so a crash from OTA'd code is readable)

An OTA bundle is JavaScript Sentry has never seen minified. Without its
sourcemap uploaded, a crash inside code that arrived over the air reports a
stack trace of minified identifiers and bundle offsets — not unreadable in the
sense of missing, unreadable in the sense of useless for finding the line that
broke. This is a *different* upload from the one `sentry.gradle` already does
during `eas build` (see docs/mobile/eas-build-profiles.md, "Two ways to break a
fingerprint" and its neighbours): that one covers the JS baked into the native
binary at build time; this one covers the JS an `eas update` publish replaces
it with, which the binary-time upload never sees.

```bash
# From apps/mobile, immediately after the eas update call for the SAME
# channel (it exported into dist). Confirm the maps exist first: no .map
# files means nothing to upload, and the crash reports stay unreadable.
find dist -name '*.map' | head
npx sentry-expo-upload-sourcemaps dist
```

Reads `SENTRY_ORG`, `SENTRY_PROJECT` and `SENTRY_AUTH_TOKEN` from the
environment — the same three the `production`/`preview` EAS environments
already hold for the build-time upload (docs/mobile/eas-build-profiles.md).
Pull them locally with `eas env:pull --environment <preview|production>`
before running the command from a machine that is not an EAS worker.

**Guard it on `SENTRY_AUTH_TOKEN` being present, and never print the token.**
`@sentry/react-native`'s own script degrades safely — it logs a warning and
skips the upload rather than failing the publish — so there is no reason to
route around that behaviour with a custom check that might get it wrong. If
scripting this step (rather than running it by hand per steps 5a/6a), guard the
...
if [ -n "$SENTRY_AUTH_TOKEN" ]; then
  ...
  echo "SENTRY_AUTH_TOKEN not set — skipping OTA sourcemap upload"
fi
```

A skipped upload does not block the publish — the OTA still reaches devices —
it only means the *next* crash from that bundle is unreadable until someone
uploads it after the fact.
