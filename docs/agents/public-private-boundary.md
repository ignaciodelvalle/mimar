...
6. **A path into one person's machine** in any tracked file's content: a
   Windows profile (`C:\Users\<name>`, any drive, either slash, escaped or
   not), a macOS home (`/Users/<name>/`) or a Linux home (`/home/<name>/`). It
   leaks a username and a machine layout. Placeholder names (`...`, `<user>`,
   `$USER`, `%USERNAME%`, `you`, `Public`, …), CI build homes (`/home/runner/`,
   `/home/expo/`, …) and paths inside an `http(s)://` URL are not findings.
