...
//   machine_path             a path into ONE person's machine in any tracked
//                            file's content: a Windows profile (C:\Users\<name>,
//                            any drive letter, either slash, escaped `\\` too,
//                            or the Git Bash /c/Users/<name> and WSL
//                            /mnt/c/Users/<name> spellings), a macOS home
//                            (/Users/<name>, capital U) or a Linux home
//                            (/home/<name>). The public tree once carried
//                            the maintainer's profile path in an e2e helper — a
//                            username and a machine layout. Not findings: a
//                            placeholder name (`...`, `<user>`, `$USER`,
//                            `%USERNAME%`, `:id`, `you`, `Public`, `Default`, …),
//                            a CI or build-image home (runner, expo, node,
//                            vscode, linuxbrew, pwuser, ubuntu, circleci,
//                            codespace), and anything inside an http(s):// URL
//                            (a site's /Home/ or /home/ path segment is not a
//                            machine). Out of scope: UNC and WSL shares
//                            (`\\wsl$\…`, `\\server\…`) and `~`-relative paths,
//                            which name no user.
//
...
/**
 * Home-directory shapes. Windows: a drive letter not glued to a word, then
 * `Users` between separators that may be `/`, `\` or an escaped run of
 * backslashes (Windows paths are case-insensitive, so `users` counts too).
 * macOS and Linux: case-SENSITIVE, an absolute path (the leading `/` not glued
 * to a path word, so `components/Users/x/` is a directory in a repo, not a
 * home). Every form's name ends at `/`, `\`, whitespace, a quote, a backtick,
 * `,`, `;`, `:` or the end of the line, so `HOME=/home/<x>` and `"/Users/<x>"`
 * are caught without a trailing slash.
 */
const WINDOWS_PROFILE =
  /(?<![A-Za-z0-9])[A-Za-z]:(?:\\+|\/)[Uu][Ss][Ee][Rr][Ss](?:\\+|\/)([^\\/\s"'`,;:]+)(?=[\\/\s"'`,;:]|$)/g;
/** The same profile as Git Bash (`/c/Users/<x>`) or WSL (`/mnt/c/Users/<x>`) spells it. */
const POSIX_DRIVE_PROFILE =
  /(?<![A-Za-z0-9._~-])(?:\/mnt)?\/[A-Za-z]\/[Uu][Ss][Ee][Rr][Ss]\/([^\\/\s"'`,;:]+)(?=[\\/\s"'`,;:]|$)/g;
const UNIX_HOME = /(?<![A-Za-z0-9._~-])\/(Users|home)\/([^\\/\s"'`,;:]+)(?=[\\/\s"'`,;:]|$)/g;

/**
 * An http(s) URL on the line: its path segments are a website's, not a
 * machine's. It ends at whitespace, a quote, a backtick or a comma, so on a
 * one-line JSON document it does not swallow the rest of the line.
 */
const HTTP_URL = /https?:\/\/[^\s"'`,]+/gi;

/** Names a doc or a template writes where somebody's username goes. */
const PLACEHOLDER_USERS = new Set(["you", "user", "username", "name", "me", "public", "default"]);

/** Homes of CI runners and build images — nobody's personal machine. */
const CI_HOMES = new Set([
  "runner",
  "expo",
  "node",
  "vscode",
...
/** Whether a captured home-directory name is a placeholder, not a person. */
export function isPlaceholderUser(name: string): boolean {
  if (/^(?:\.{2,}|…+|\*+)/.test(name)) return true;
  // `<x>`, `{x}`, `[x]`, `$X`, `%X%`, and a route parameter such as `:id`.
  if (/^[<{[$%:]/.test(name)) return true;
  const lower = name.replace(/[.,;:)]+$/, "").toLowerCase();
  if (PLACEHOLDER_USERS.has(lower)) return true;
  return /^your(?:[-_]|name|user)/.test(lower);
}

/** The first machine path on the line, as `C:/Users/<name>`-style text, or null. */
export function machinePathOn(line: string): string | null {
  if (!/[Uu][Ss][Ee][Rr][Ss]|home/.test(line)) return null;
  const text = line.replace(HTTP_URL, " ");
  for (const m of text.matchAll(WINDOWS_PROFILE)) {
    if (!isPlaceholderUser(m[1])) return "a Windows user-profile path";
  }
  for (const m of text.matchAll(POSIX_DRIVE_PROFILE)) {
    if (!isPlaceholderUser(m[1])) return "a Windows user-profile path (Git Bash / WSL form)";
  }
  for (const m of text.matchAll(UNIX_HOME)) {
    if (isPlaceholderUser(m[2])) continue;
    if (m[1] === "home" && CI_HOMES.has(m[2])) continue;
    return m[1] === "Users" ? "a macOS home-directory path" : "a Linux home-directory path";
  }
  return null;
}
...
export function findContent(
  ...
  lines.forEach((line, i) => {
    ...
    const machine = machinePathOn(line);
    if (machine) {
      findings.push({ path, line: i + 1, kind: "machine_path", detail: machine });
    }
...
function remedy(f: BoundaryFinding): string {
  switch (f.kind) {
    ...
    case "machine_path":
      return "derive the path at runtime (os.homedir(), an env var, a repo-relative path) or write a placeholder (C:/Users/<you>/…)";
  }
}
