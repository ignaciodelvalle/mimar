...
// (localidades-por-id C2, PO decisions D1 + D2). Pure: no database, so the
// rules are unit-tested; scripts/seed-authority-units.ts feeds it the live
// catalogue and writes what it returns.
//
// D1 — units come from INDEC departments:
//   - Buenos Aires: a department IS a partido, and a partido is a municipio.
//   - Every other province: a department is not a municipality, so the unit
//     is proposed as kind `departamento`. Every seeded unit is a DRAFT; a
//     platform admin confirms each with the authority (C4 editor).
...
//   - A locality without a department outside CABA is NOT placed. It is
//     reported for an admin, never attached to a guessed unit (P1).
...
export type AuthorityUnitPlan = {
  units: PlannedUnit[];
  /** Rows the plan refuses to place (no department outside CABA). */
...
export function isGovernableLocality(row: CatalogueLocality): boolean {
...
export function planAuthorityUnits(catalogue: readonly CatalogueLocality[]): AuthorityUnitPlan {
