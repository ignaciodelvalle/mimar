...
const RED: Record<BoundaryKind, { path: string; src: string | null }> = {
  ...
  machine_path: {
    path: "e2e/_base-url.ts",
    src: cat('const envFile = "C:', "\\", "Users", "\\", "fixtureuser", '\\dev\\.env";'),
  },
};
...
describe("machine paths — one person's home directory, not a placeholder or a CI box", () => {
  // Every sample is assembled so this file's own text never carries a match.
  const U = cat("Us", "ers");
  const H = cat("ho", "me");
  it.each([
    ["Windows, backslashes", cat("C:\\", U, "\\fixtureuser\\dev\\dim")],
    ["Windows, forward slashes", cat("D:/", U, "/fixtureuser/dev/dim")],
    ["Windows, escaped backslashes", cat('"C:\\\\', U, '\\\\fixtureuser\\\\dev"')],
    ["Windows, lowercase drive and folder", cat("c:/", U.toLowerCase(), "/fixtureuser/x")],
    ["Windows, bare profile at end of line", cat("cd C:\\", U, "\\fixtureuser")],
    ["macOS", cat("open /", U, "/fixtureuser/code/dim")],
    ["Linux", cat("cp x /", H, "/fixtureuser/dim/.env.local")],
    ["a file:// URL", cat("file:///", U, "/fixtureuser/report.html")],
    ["Git Bash drive form", cat("cd /c/", U, "/fixtureuser/dev/dim")],
    ["WSL drive form", cat("cp /mnt/c/", U, "/fixtureuser/dev/x .")],
    ["Linux, no trailing slash", cat("HOME=/", H, "/fixtureuser")],
    ["macOS, quoted, no trailing slash", cat('const dir = "/', U, '/fixtureuser";')],
    ["Windows, name ended by a comma", cat('["C:/', U, '/fixtureuser", "x"]')],
    [
      "a machine path after a URL on a one-line JSON document",
      cat('{"url":"https://example.org/a","dir":"/', H, '/fixtureuser/dim"}'),
    ],
  ])("RED: %s fires", (_, line) => {
    expect(findContent("lib/x.ts", line).map((f) => f.kind)).toEqual(["machine_path"]);
  });

  it.each([
    ["an ellipsis", cat("C:/", U, "/.../dim/.env.local")],
    ["a unicode ellipsis", cat("C:\\", U, "\\…\\dim")],
    ["an angle placeholder", cat("C:\\", U, "\\<you>\\dim")],
    ["a %USERNAME% variable", cat("C:\\", U, "\\%USERNAME%\\dim")],
    ["a $USER variable", cat("/", H, "/$USER/dim")],
    ["a ${USER} variable", cat("/", U, "/${USER}/dim")],
    ["the word you", cat("/", U, "/you/dim")],
    ["your-user", cat("/", H, "/your-user/dim")],
    ["USERNAME", cat("C:/", U, "/USERNAME/dim")],
    ["name", cat("/", U, "/name/dim")],
    ["me", cat("/", H, "/me/dim")],
    ["Public", cat("C:\\", U, "\\Public\\Documents")],
    ["Default", cat("C:\\", U, "\\Default\\AppData")],
    ["GitHub Actions' runner home", cat("/", H, "/runner/work/dim/dim")],
    ["EAS builds' expo home", cat("/", H, "/expo/workingdir/build")],
    ["the node image home", cat("/", H, "/node/app")],
    ["a devcontainer home", cat("/", H, "/vscode/.cache")],
    ["linuxbrew", cat("/", H, "/linuxbrew/.linuxbrew/bin")],
    ["the Playwright image home", cat("/", H, "/pwuser/.cache/ms-playwright")],
    ["an ubuntu image home", cat("HOME=/", H, "/ubuntu")],
    ["a CircleCI home", cat("/", H, "/circleci/project")],
    ["a Codespaces home", cat("/", H, "/codespace/.vscode-remote")],
    ["a route parameter", cat("GET /", U, "/:id/pets")],
    ["a Git Bash placeholder", cat("/c/", U, "/<you>/dev")],
    ["a WSL placeholder", cat("/mnt/c/", U, "/.../dev")],
    ["a website's /Home/ segment", cat("https://www.example.org/fileadmin/Home/eng/x.pdf")],
    ["a website's /home/ segment", cat("see https://example.org/", H, "/fixtureuser/page")],
    ["a website's /Users/ segment", cat("https://api.example.org/", U, "/fixtureuser/profile")],
    ["a repo directory named Users", cat("components/", U, "/fixtureuser/Card.tsx")],
    ["a relative home dir", cat("fixtures/", H, "/fixtureuser/x.json")],
    ["a lowercase macOS-like path", cat("/", U.toLowerCase(), "/fixtureuser/x")],
  ])("stays green for %s", (_, line) => {
    expect(findContent("lib/x.ts", line), line).toEqual([]);
  });

  it("isPlaceholderUser decides on the captured name", () => {
    for (const n of ["...", "…", "<user>", "%USERNAME%", "$USER", "${USER}", "Default", ":id"]) {
      expect(isPlaceholderUser(n), n).toBe(true);
    }
    for (const n of ["fixtureuser", "runner", "juan"]) {
      expect(isPlaceholderUser(n), n).toBe(false);
    }
  });

  it("names one finding per line even when a line carries two shapes", () => {
    const line = cat("C:/", U, "/fixtureuser/x and /", H, "/fixtureuser/y/");
    expect(machinePathOn(line)).toBe("a Windows user-profile path");
    expect(findContent("lib/x.ts", line)).toHaveLength(1);
  });
});
