...
describe("reportBiteFromOrg — the incident keeps its place (A8)", () => {
...
describe("reportBiteFromOrg — an unresolved pin is never filed at the home pair", () => {
  it("opens the case with no jurisdiction and keeps the place on the incident", async () => {
    ...
    await reportBiteFromOrg({ ...BASE_INPUT, eventPlace: place }, deps);
