...
describe("rehome — every writer that ENDS custody takes the pet advisory lock first", () => {
  ...
  const LOCK_IS_THE_CALLERS_DUTY: Record<string, string> = {
    ...
    "src/modules/transfers/infrastructure/transfers-repository.ts":
      "closeOwnerOwnerships is a pass-through delegate to endCaretakerArrangementsForPet, called from acceptPetTransfer's transaction. That transaction takes acquirePetAdvisoryLock as its first statement (audit K, W4 — the residual this entry used to record), then serialises on the transfer row (findTransferByIdForUpdate) and re-asserts the sender is still the single active owner. Pinned by src/modules/transfers/application/__tests__/use-cases.test.ts (call order) and __tests__/custody-handoff-pet-lock-races.test.ts (a real second connection).",
