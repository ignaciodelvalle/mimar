...
//
// PppAttestationRegistriesForm's `notes` had a STATIC
// defaultValue={initialNotes} — a rejected submit put back the row's
// ORIGINAL notes, discarding whatever the operator had just typed.
//
...
describe("<PppAttestationRegistriesForm> — survives the React 19 post-error reset", () => {
  it("keeps the typed notes after a rejected submit", async () => {
    ...
    expect((container.querySelector('textarea[name="notes"]') as HTMLTextAreaElement).value).toBe(
      "Pendiente confirmar con el registro provincial.",
    );
  });
});
