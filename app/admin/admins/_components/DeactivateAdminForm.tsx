  ...
  function submit() {
    ...
    startTransition(async () => {
      ...
      if ("error" in result) {
        ...
        // navigateAfterActionSuccess() is a full window.location.assign(),
        // which discards this whole React tree anyway. Calling onDone()
        // triggered a setMode("done") re-render of the parent a beat before
        // the browser actually unloaded — a stray render of a stale tree,
        // visible as a flash right before the full reload replaces it.
        // onDone stays in the props contract for a non-navigating success
        // path, if one is ever added; there isn't one today.
