...
//
// /admin/moderacion — fix (adversarial-admin 2026-07-23): this route now only
// redirects into the Denuncias hub's "moderacion" stage, preserving every
// query param — mirrors /gob/moderacion's F1 fusion redirect. Regression
// guard: a bookmarked/shared old-route URL must land on the exact same slice
// of data under /gob/denuncias.
...
import AdminModeracionRedirectPage from "./page";

describe("/admin/moderacion — redirects into the Denuncias hub", () => {
  it("redirects to /gob/denuncias?etapa=moderacion with no other params", async () => {
    await AdminModeracionRedirectPage({ searchParams: Promise.resolve({}) });
  ...
  it("preserves status/kind/severity/cursor and sets etapa=moderacion", async () => {
    await AdminModeracionRedirectPage({
